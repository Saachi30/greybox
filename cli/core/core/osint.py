...
# crt.sh is a free public service that's frequently slow/overloaded -
# read timeouts and 502/503 are routine, not exceptional, so this treats
# them as expected transient conditions worth retrying rather than letting
# them crash straight through as an unhandled exception (which is exactly
# what was happening before - the network call had no try/except around
# it at all).
...
def crtsh_lookup(values: dict[str, str]) -> str:
    ...
    flaky/slow under load, so this retries several times (including on
    read timeouts) before giving up rather than treating the first failure
    as final.
    """
    domain = values.get("domain", "")
    last_error = None
    ...
    for attempt in range(_MAX_ATTEMPTS):
        try:
            ...
            # Covers ReadTimeout, ConnectTimeout, ConnectionError, etc. -
            # this is the branch that was previously missing entirely,
            # letting a plain timeout propagate as a raw, unhandled
    ...
    return (
        f"crt.sh didn't return usable results for {domain} after {_MAX_ATTEMPTS} attempts "
